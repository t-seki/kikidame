...
- **Sync** — per program, choose whether sync is enabled for it and its retention rule. WorkManager syncs every 6 hours, with a "Wi-Fi only" option. While a background sync runs, a thin bar under the top bar says so. After a sync, a snackbar shows only what changed (new episodes, queued downloads, deleted episodes); a background sync with no changes shows nothing. Episodes you download by hand are Pinned and never removed by the rules
