...
- **番組一覧** — 「よく聴く」の印で節を分け、番組名・放送局名の検索と放送局での絞り込み
- **各回一覧** — ダウンロード／固定／再生済みの操作。手元にあるか、サーバから消えたか（消失）を行に出す
...
- 想定しているフォルダ構成は [radirec-tool](https://github.com/t-seki/radirec-tool) の出力（`<放送局>/<番組>/<番組> YYYY-MM-DD.m4a`、albumartist に放送局、`©day` に放送日）。同じ形にタグを付けてあれば他のツールでもよい
