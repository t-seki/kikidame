...
2026-09-17 に Pixel 7a（Android 17）と Jellyfin 10.11.11（`https://jellyfin.lab.tseki.net`）で確認済み。
...
- [ ] 「この番組を今すぐ同期」が他の番組に触れず数秒で終わる。番組一覧の引っ張りは全体同期で「番組 153 / 各回 N を取得…」
- [x] 「この番組を今すぐ同期」で最新 3 回がキューに入り（ピン無し）、順に落ちる。スナックバーに「3 回をダウンロード予約」（2026-09-18、全体同期経由で確認）
- [ ] 同期分が落ちている最中に別の回を手動で落とすと、手動の回が先に落ちる
- [ ] サーバに 4 本目が入って同期すると、3 本目（未再生）が消えて 4 本目が落ちる。消えた回の再生位置は残る（落とし直すと続きから）
- [ ] 固定した回は N の外で残る。固定を 3 本以上しても最新 3 回は別枠で落ちる
- [ ] 「再生済みなら削除」ON で聴き終えた回が次の同期で消える。再生済みのまま落とし直されない
- [ ] 保持されるはずの回を手で削除すると「次の同期で落とし直されます」と出て、同期で戻ってくる
- [ ] 同期 OFF にすると確認ダイアログが出る。確定すると次の同期で非固定の回が全部消え、固定は残る
- [x] 再生中の回が削除対象でも、その同期では消えない（次の同期で消える）（2026-09-18: N を 3 → 1 にして確認。再生位置 13 秒が残った）
- [ ] サーバ側で各回を消して同期すると、固定していても一覧から消える。番組ごと消える（ライブラリ切替でも可）と「N 番組はサーバ上で見つからず」と出て何も消えない
- [ ] サーバを止めて同期すると「サーバに接続できません」で何も消えず何も落ちない
- [ ] 「Wi-Fi のみ」ON でモバイル回線で引っ張ると「Wi-Fi に接続していないため更新しません」。OFF にすると取得する
- [ ] 各回一覧の引っ張って更新が 1 秒前後で終わり、その番組の新しい回だけ増える（#12）。サーバから消えた回は消えない
- [x] 起動時に `sync-once` が走る（logcat「sync: 番組 153 / 各回 6003 を取得」）。`dumpsys jobscheduler` に `sync-periodic` が登録され、
  前面に戻しても次回実行（Minimum latency ≈ 6h）がリセットされない（2026-09-18）
