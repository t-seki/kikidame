...
- [ ] 再開（#108）: 再生を止めてアプリのプロセスを殺し（`adb shell am force-stop dev.tseki.kikidame`）、DHU の Kikidame の ▶ か「最近」（端末の再起動直後にシステムが出す再開の候補）から、最後に聴いていた回がその番組のキューごと保存位置から始まる（`onPlaybackResumption`）
