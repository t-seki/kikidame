...
同期の入口は 1 つ（`LibraryRefresher`）。番組一覧を引っ張る＝手動同期、各回一覧を引っ張る＝その番組だけ取り込む（削除なし）。
...
- [ ] 各回一覧の同期アイコン → シートで「この番組を同期する」を ON にすると「最新 3 回」が選ばれ、番組一覧の行に同期の印が付く
- [ ] 「今すぐ同期」で最新 3 回がキューに入り（ピン無し）、順に落ちる。スナックバーに「3 回をダウンロード予約」
...
- [ ] 再生中の回が削除対象でも、その同期では消えない（次の同期で消える）
...
- [ ] アプリを 1 時間以上閉じて開くと `sync-once` が走る（logcat に SyncWorker）。`dumpsys jobscheduler` に `sync-periodic` が登録されている
