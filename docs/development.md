...
サーバ上に無い番組を作る手段: (a) 設定でライブラリを切り替える（旧ライブラリの番組が全部消失になる）、(b) サーバ側で 1 番組のフォルダを外してスキャン。
(a) が手軽。切り替えた後に番組一覧を引っ張ると「N 番組はサーバ上で見つからず、そのままにしました」と出る。
- [ ] 消失した番組の行に「サーバ上で見つかりません」と `CloudOff` アイコンが付き、並び順は変わらない
- [ ] その番組の同期シートでスイッチが無効になり「サーバ上で見つかりません（YYYY-MM-DD から）。同期は止まっています…」と出る。「この番組を今すぐ同期」は出ず「この番組を手元から消す」が出る
- [ ] 消失した番組の手元の回はそのまま再生できる
- [ ] 「この番組を手元から消す」→ 確認 → 番組一覧に戻り、番組・ファイルが消えている
- [ ] 元のライブラリに戻して同期すると印が消える（`goneSince` が null に戻る）
- [ ] 設定画面・接続画面にデバッグ節（シード）が出ない（#21）
