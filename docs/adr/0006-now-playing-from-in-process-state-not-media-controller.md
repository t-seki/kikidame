...
- Media3 は `MediaButtonReceiver` の Manifest 宣言があるときだけ再開（SystemUI の `isRecent` と `onPlaybackResumption`）を有効にする（`MediaSessionLegacyStub.canResumePlaybackOnStart`。#119）。SystemUI の `isRecent` はアプリの `onGetLibraryRoot` を経由せず Media3 が自前のルートを返すので、コールバック側では制御できない
