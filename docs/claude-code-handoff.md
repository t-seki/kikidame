...
2 本の PR に分ける。**PR 1 = #2 + #9（突合の拡張、ADR 0005）** → 実機（#9 の再現データ）→ マージ → **PR 2 = #3（消失の記録と UI）**。
...
- **第二段（#9）**: タイトルで結べなかった未結合の各回に対し、同じ番組内で **放送日（日単位）が同じ ＋ 尺の差が 5 秒以内** の候補が双方 1 対 1 なら結ぶ。
...
- **実機確認**: #9 は端末に再現データがある（パンサー向井のふらっと: シード 6 本 + サーバ 6 本）ので、同期後に 1 行ずつに結ばれ再生位置とファイルが残ることを確認。
  #2 の再取り込みは自宅ライブラリを作り直す必要があるため **Robolectric のみ**（`FakeJellyfinGateway` で番組 ID・各回 ID の変更を模す）
