...
- 同期中の表示は 2 種類で、両方は出さない。利用者の操作（引っ張って更新・「この番組を今すぐ同期」）はクルクル（`PullToRefreshBox`）、裏の同期（定期・起動時）は番組一覧・各回一覧のトップバーの下に細い不確定のバーと「バックグラウンドで同期中…」（`ui/BackgroundSyncBar.kt`、#138）。裏の同期の最中に操作が来たらクルクルに切り替える（#134）
