...
| 画面 | 役割 | #48 で変えること |
| --- | --- | --- |
| 番組一覧 | 聴く番組を選ぶ根の画面。「よく聴く／その他」の節、行に `配信元 · 未再生 N / 手元 L · 全 E 回 · 最新 MM-DD`（#41、変えない） | 絞り込みの入口を 1 つに統合（#55）。検索（#44）と配信元（#45）は 1 枚のシートに、選択中は検索語と配信元をそれぞれチップで |
...
- ジャケット・波形など、データに無い視覚要素で埋めない（番組型の音声にジャケットが無いことは多い。偽の波形も置かない）

### 文言の足し方（#104、ADR 0009）

UI の文言は英語と日本語の 2 言語で、OS の「アプリの言語」（Android 13+）で切り替わる。既定は英語（日英以外の端末には英語が出る）。

- 文言は `app/src/main/res/values/strings.xml`（英語、既定）と `values-ja/strings.xml`（日本語）の**両方**に足す。片方だけだと lint の `MissingTranslation` でビルドが落ちる。`values/` にはキーごとに翻訳者向けの短いコメント、書式引数は `%1$s` のように番号付き、数+単位（「3 回」「15 分」）は `<plurals>`
...
| B 番組表 | 配信元を最上位に。配信元タブ・配信元ごとの色 | 「よく聴く」の節。配信元が 1 つの利用者には効かない。配信元名の表記ゆれで色が割れる |
