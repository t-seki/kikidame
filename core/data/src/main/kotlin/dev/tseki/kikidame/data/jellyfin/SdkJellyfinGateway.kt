...
class SdkJellyfinGateway @Inject constructor(
    ...
    private fun BaseItemDto.toServerEpisode(): ServerEpisode? {
        ...
        return ServerEpisode(
            ...
            sizeBytes = null,
            container = container ?: "",
            // 出演者（#70）。`Artists` は基本フィールドで返る（`ItemFields` に無く、要求の有無に依らない）。名前だけの一覧が無い・空なら `ArtistItems` から拾う
            performers = (artists?.takeIf { it.isNotEmpty() } ?: artistItems?.mapNotNull { it.name }).orEmpty().filter { it.isNotBlank() },
        )
    }
    ...
    companion object {
        const val CLIENT_NAME = "Kikidame"
