 ...
 * 1 番組分の同期の結論。
 *
 * @property download 保持すべきなのに手元に無い各回（放送日の新しい順）
...
/** 全番組分をまとめたもの。[download] は番組をまたいで放送日の新しい順。 */
data class SyncPlan(
 ...
 * - 同期対象なら、固定でないサーバ上の各回を放送日の新しい順に並べ、N 回まで取り、「再生済みなら削除」なら再生済みを除いたものが保持すべき集合。
...
object SyncPlanner {
    ...
    fun plan(inputs: List<SyncProgramInput>): SyncPlan {
        ...
        val airedAtById = inputs.asSequence().flatMap { it.local }.associate { it.id to (it.airedAt to it.title) }
        val download = plans.flatMap { it.download }
            .sortedWith(compareByDescending<EpisodeId> { airedAtById.getValue(it).first }.thenBy { airedAtById.getValue(it).second }.thenBy { it.value })
        return SyncPlan(plans, download)
    }

    /** [EpisodeOrder.newestFirst] と同じ順（放送日の新しい順 → タイトル → ローカル ID）。 */
    private val NEWEST_FIRST: Comparator<LocalEpisodeState> =
        compareByDescending<LocalEpisodeState> { it.airedAt }.thenBy { it.title }.thenBy { it.id.value }
}
